...
import by.boginsky.util.constants.ConstantLocales;
...
public class MessageManager {
    ...
    public static MessageManager defineLocale(String locale){
        ...
        if(locale == null){
            messageManager = new MessageManager(ResourceBundle.getBundle(ConstantBundles.MESSAGE, new Locale(ConstantLocales.RUSSIAN_LOCALE)));
        }else {
            switch (locale){
                case ConstantLocales.ENGLISH_LOCALE:
                    messageManager = new MessageManager(ResourceBundle.getBundle(ConstantBundles.MESSAGE,
                            new Locale(ConstantLocales.ENGLISH_LOCALE, ConstantLocales.US)));
                    break;
                case ConstantLocales.RUSSIAN_LOCALE:
                    messageManager = new MessageManager(ResourceBundle.getBundle(ConstantBundles.MESSAGE,
                            new Locale(ConstantLocales.RUSSIAN_LOCALE,ConstantLocales.RUSSIA)));
            }
        }
